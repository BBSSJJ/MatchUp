...
public class SummonerController {
    ...
    @Operation(summary = "소환사 리그정보 조회(SummonerName)", description = "소환사 이름으로 소환사정보와 리그정보를 불러오는 API 입니다.") // 해당 API가 어떤 역할을 하는지 설명
    @ApiResponses(value = {
    ...
    public ResponseEntity<SummonerLeagueInfoResponseDto> getSummonerInfoBySummonerName(
        ...
        SummonerLeagueInfoResponseDto summonerLeagueInfo = summonerService.getSummonerDefaultInfoBySummonerName(summonerName);
        return ResponseEntity.ok(summonerLeagueInfo);
    }

    @Operation(summary = "소환사 리그정보 조회(Game Name, Tag Name)", description = "라이엇 이름과 태그로 소환사정보와 리그정보를 불러오는 API 입니다.") // 해당 API가 어떤 역할을 하는지 설명
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "소환사 + 리그정보", // 응답코드 200일때 응답 설명
                    content = @Content(schema = @Schema(implementation = SummonerLeagueInfoResponseDto.class))), // 해당 응답코드에서 어떤 클래스를 응답하는지 작성
            @ApiResponse(responseCode = "400", description = "잘못된 요청", // 응답코드 400일때 응답 설명
    ...
    public ResponseEntity<SummonerLeagueInfoResponseDto> getSummonerInfoByGameNameAndTag(
        ...
        SummonerLeagueInfoResponseDto summonerLeagueInfo = summonerService.getSummonerDefaultInfo(gameName, tagLine);
        return ResponseEntity.ok(summonerLeagueInfo);
    }
    ...
    public ResponseEntity<SummonerLeagueInfoResponseDto> getSummonerInfoByPuuid(
        ...
        SummonerLeagueInfoResponseDto summonerLeagueInfo = summonerService.getSummonerDefaultInfo(puuid);
        return ResponseEntity.ok(summonerLeagueInfo);
    }
}