...
public class SummonerUserController {
    ...
    public ResponseEntity<SummonerInfoResponseDto> registSummonerInfo(
    ...
    @Operation(summary = "소환사 로그인 시 지표 갱신 후 라이엇 정보 반환(SummonerId)", description = "로그인 시 DB에 없는 매치정보가 있는지 확인하여 갱신 후, 해당 라이엇 계정 정보를 반환합니다.") // 해당 API가 어떤 역할을 하는지 설명
    @ApiResponses(value = {
    ...
    @PostMapping("/summoner-id/{summonerId}")
    public ResponseEntity<SummonerInfoResponseDto> loginSummonerInfo(
