...
- **webserver** : 프론트엔드 웹서버를 구현하는 브랜치
- **appserver** : 백엔드 어플리케이션 서버를 구현하는 브랜치
- **fastapi** : 빅데이터 서버를 구현하는 브랜치
- **auth** : 인증 서버를 구현하는 브랜치
- 기타 추가 서버 구축 및 작업 시, 새로운 브랜치 생성 후 필요시 CI / CD 적용
- **feature** : 각 서버별 구체적인 기능을 구현하는 브랜치, 작업 후 해당 서버 브랜치로 병합
